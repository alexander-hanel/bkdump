...
pslist* GetProcessList( )
{
    ...
    PROCESSENTRY32 pe32;
    int i = 0;
    TCHAR psName[][20] = { "firefox.exe", "IEXPLORE.EXE", "iexplore.exe", "chrome.exe", "explorer.exe"};
    pslist* head, *curr; 
    head = NULL;
    ...
    do
      {
        ...
        for ( i = 0; i < 5 ; i++)
        {
            if( strcmp(psName[i], pe32.szExeFile ) == 0)
            {
...
// scan a pid for memory that is marked as RWX and is greater than 4096. 
// if the pid is equal to zero open up a dummy iexplore.exe process 
...
// for each mb (memory block) create a file with the Pid.0xbaseaddress.bin in the working directory 
